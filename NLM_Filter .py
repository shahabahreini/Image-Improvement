...
def load_rgb_geotiff(image_path):
    ...
    if image.shape[2] >= 3:
        ...
        raise ValueError(f"Image only has {image.shape[2]} channels, need at least 3 for RGB")
...
def apply_nlm_rgb_ultrafast(image):
    ...
    if h > 2000 or w > 2000:
        ...
        denoised = rescale(denoised_small, 1/scale, anti_aliasing=False, channel_axis=-1)
...
def save_to_geotiff(filtered_rgb, profile, transform, crs, output_path):
    ...
    print(f"✓ Filtered image range: {filtered_rgb.min():.3f} to {filtered_rgb.max():.3f}")
